...
window.addEventListener('DOMContentLoaded', function () {
  'use strict'; // объект с данными калькулятора
  ...
  Object(_modules_tabs__WEBPACK_IMPORTED_MODULE_2__["default"])('.glazing_slider', '.glazing_block', '.glazing_content', 'active'); // в соответствии с версткой класс табов выбран no_click, т.к остальные другие меняют свое название. Это не ок
  ...
  Object(_modules_timer__WEBPACK_IMPORTED_MODULE_5__["default"])('.container1', deadline); // images();
});
...
var changeModalState = function changeModalState(state) {
  ...
  var windowForm = document.querySelectorAll('.balcon_icons_img'); // селектор с табами

  var windowWidth = document.querySelectorAll('#width'); // Тут лежит 1 элемент, но псевдомассив нужен для отработки функции

  var windowHeight = document.querySelectorAll('#height'); // аналогично

  var windowType = document.querySelectorAll('#view_type'); // аналогично

  var windowProfile = document.querySelectorAll('.checkbox');
  Object(_checkNumInputs__WEBPACK_IMPORTED_MODULE_1__["default"])('#width');
  Object(_checkNumInputs__WEBPACK_IMPORTED_MODULE_1__["default"])('#height'); // передача данных в state
    ...
    elem.forEach(function (item, i) {
      ...
      item.addEventListener(event, function () {
        switch (item.nodeName) {
          case 'SPAN':
            // название элемента из nodeName всегда в верхнем регистре
          ...
          case 'INPUT':
            if (item.getAttribute('type') === 'checkbox') {
              ...
              i === 0 ? state[prop] = 'Холодное' : state[prop] = 'Теплое'; // нужно убрать также галочки у всех чекбоксов, кроме нужного
  ...
  bindActionToElems('click', windowForm, 'form'); // запись высоты

  bindActionToElems('input', windowWidth, 'width'); // запись ширины

  bindActionToElems('input', windowHeight, 'height'); // запись типа остекления

  bindActionToElems('change', windowType, 'type'); // запись профиля
...
var checkNumInputs = function checkNumInputs(selector) {
  var numInputs = document.querySelectorAll(selector); // только числа в полях для телефоного номера
...
var forms = function forms(state) {
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      ...
      item.appendChild(statusMessage); // сбор данных в форме через FormData

      var formData = new FormData(item); // для формы в калькуляторе с данными из него. Динамические данные
      ...
      postData('assets/server.php', formData).then(function (res) {
        ...
        setTimeout(function () {
          Object(_modal__WEBPACK_IMPORTED_MODULE_6__["closeAllModals"])(windows);
        }, 2000);
      }).catch(function () {
...
var images = function images() {
  ...
  workSection.appendChild(imgPopup); // 3 варианта задания стилей. Через класс в css, через инлайн стили, черезе cssText
...
/*!*********************************!*\
  !*** ./src/js/modules/modal.js ***!
  \*********************************/
/*! exports provided: default, closeModal, closeAllModals, openModal */
/***/ (function(module, __webpack_exports__, __webpack_require__) {
...
var modal = function modal(modalState) {
  ...
  }, 60000); // кнопка вызов замерщика

  bindModal('.popup_engineer_btn', '.popup_engineer', '.popup_engineer .popup_close', modalTimerId); // кнопка заказать обратный звонок

  bindModal('.phone_link', '.popup', '.popup .popup_close', modalTimerId); // кнопка рассчитать стоимость

  bindModal('.popup_calc_btn', '.popup_calc', '.popup_calc_close', modalTimerId); // кнопка далее в калькуляторе. Тут также блокируется закрытие по клику на подложку

  bindModal('.popup_calc_button', '.popup_calc_profile', '.popup_calc_profile_close', modalTimerId, false); // кнопка далее в выборе профиля
...
var tabs = function tabs(headerSelector, tabSelector, contentSelector, activeClass) {
  ...
  hideTabContent();
  showTabContent(); // Отслеживание табов, на которые кликнули через делегирование событий

  header.addEventListener('click', function (e) {
    var target = e.target; // проверка на правильность клика по табу и только после этого можно их перебирать

    if (target && ( //  наличие вообще таргета у элемента
...
var timer = function timer(id, deadline) {
  ...
  var getTimeRemaining = function getTimeRemaining(endTime) {
    var t = Date.parse(endTime) - Date.parse(new Date()); // разница между дедлайном и текущей датой
  ...
  var setClock = function setClock(selector, endTime) {
    ...
    var timeInterval = setInterval(updateClock, 1000); // вызов обновления таймера до запуска интервала (чтобы не мигала верстка)
